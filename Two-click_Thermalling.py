...
#Fns to convert between degrees and radians
#Radians to degrees
def rad_deg(theta):
    phi = theta*(180/(np.pi))
    return phi
...
def Taubin_Newton (x_pos, y_pos):
    ...
    if (nPoints < 3):
        ...
        #[x coordinate, y coordinate, z, radius]
        det = xnew * xnew - xnew * Mz + Cov_xy
        x = (Mxz * (Myy - xnew) - Myz * Mxy) / (det * 2)
        y = (Myz * (Mxx - xnew) - Mxz * Mxy) / (det * 2)
        centreRadius[0] = x + x_centroid #X coordinate
        centreRadius[1] = y + y_centroid #Y coordinate
        centreRadius[3] = np.sqrt(centreRadius[0]**2 + centreRadius[1]**2) #Radius

        return centreRadius

#Find the difference between 2 points, i and i+1 in an array
...
while (i<len(time_array)-1):
    if lat_array[i] == lat_array[i+1] and long_array[i] == long_array[i+1]:
        ...
        x = (np.cos(lat_array[i]) * np.sin(lat_array[i+1])) - (np.sin(lat_array[i]) * np.cos(lat_array[i+1]) * np.cos(difference(long_array)))
...
#Calculate distance between each point as the crow flies
#Create an array for the change in altitude between each point
dist_xyz = [0]
delta_z = [0]
R = 6371e3
i = 0
...
while (i<len(time_array)-1):
    speed = dist_xyz[i]/delta_t[i]
    ground_speed.append(speed)
    i = i+1

#Calculate the positin relative to the take-off location
...
#Select the points for which you want to calculate the wind vector
thermal = [0, 0, 0]
ia = 0
while ia < 2:
    fig = plt.figure()
...
#Make sure thermal[0] is the start point and thermal [1] is the end point
if thermal [0] > thermal [1]:
    thermal[2] = thermal[1]
    thermal[1] = thermal[0]
    thermal[0] = thermal[2]
...
ax.scatter3D(y_values, x_values, z_values)
plt.xlabel('East - West (m/s)')
plt.ylabel('North - South (m/s)')
ax.set_zlabel('Altitude(m)')
plt.show()

#Plot the bearing against time of the thermal
plt.plot(time_array[thermal[0]:thermal[1]], bearing[thermal[0]:thermal[1]])
plt.xlabel('Time (seconds since midnight)')
plt.ylabel('Bearing from North')
plt.show()
...
#calculate the error in the wind speed
...
#Claculate the average climb rate of the glider
z = (alt_gps_array[thermal[1]] - alt_gps_array[thermal[0]])/(time_array[thermal[1]] - time_array[thermal[0]])
wind_vector[2] = z

wind_knots = wind_vector[3]*1.9438 #Calculating the wind speed in knots
...
print('wind speed is: %5.2f m/s or %5.2f+/-%5.2f knots pointing from %5.2f from North'% (wind_vector[3], wind_knots, knots_err, wind_direction_deg))

#Plot the ground vector of the graph to show the differene between the circular regression and the mean of a set
plt.plot(x_values, y_values, 'o')
plt.plot(np.average(x_values), np.average(y_values), 'r1')
plt.plot(wind_vector[0], wind_vector[1], 'gx')
plt.xlabel('East_West ground vector component(m/s)')
plt.ylabel('North_South ground vector component(m/s)')
axes = plt.gca()
axes.set_xlim([-45,45])
axes.set_ylim([-45,45])
plt.show()
...
#Plot the actual track of the thermal in 3D with the wind vector
ax = plt.axes(projection = '3d')

ax.scatter3D(x_pos[thermal[0]:thermal[1]], y_pos[thermal[0]:thermal[1]], alt_gps_array[thermal[0]:thermal[1]])

ax.set_xlabel('East-West (m from start)')
ax.set_ylabel('North-South (m from start)')
ax.set_zlabel('Altitude (m)')
ax.quiver(np.average(x_pos[thermal[0]:thermal[1]]), np.average(y_pos[thermal[0]:thermal[1]]), np.average(alt_gps_array[thermal[0]:thermal[1]]), wind_vector[0], wind_vector[1], wind_vector[2], color='r', length=wind_vector[3])
plt.show()

#Plot the whole glider flight in 3D - this was done for fun but may find a use
#Colour application to distinguish between sections of altitude gain and loss
